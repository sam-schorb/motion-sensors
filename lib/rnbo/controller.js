...
import { clamp } from "@/lib/rnbo/mapping";
...
export function createRnboController({
  ...
  async function loadPatcher() {
    if (patcher) {
      ...
      emitDeep("patcher", {
        status: "loaded",
        url: patcherLabel || patcherUrl || "in-memory patcher",
        rnboVersion,
      });
      return patcher;
    }

    if (!patcherUrl) throw new Error("No patcher configured (patcher or patcherUrl required).");
    emitDeep("patcher", { status: "loading", url: patcherUrl });
    patcher = await loadPatcherFromUrl(patcherUrl);
    const rnboVersion = patcher?.desc?.meta?.rnboversion || null;
    emitDeep("patcher", { status: "loaded", rnboVersion, url: patcherUrl });
    log(`Loaded patcher JSON (${patcherUrl}).`);
    return patcher;
  }

  function setPatcher(nextPatcher, { label } = {}) {
    patcher = nextPatcher;
    patcherLabel = label || patcherLabel;
    const rnboVersion = patcher?.desc?.meta?.rnboversion || null;
    emitDeep("patcher", {
      status: patcher ? "loaded" : "idle",
      url: patcher ? patcherLabel || patcherUrl || "in-memory patcher" : patcherUrl || null,
      rnboVersion,
    });
    clearDevice();
  }
  ...
  function setParamNormalized(paramId, normalizedValue) {
    if (!device) return false;
    const p = device.parametersById?.get(paramId);
    if (!p) return false;
    try {
      p.normalizedValue = clamp(Number(normalizedValue), 0, 1);
      const value = p.value;
      if (state.params?.list?.length) {
        const nextList = state.params.list.map((entry) =>
          entry.id === paramId ? { ...entry, value } : entry
      ...
      log(`Failed setting normalized ${paramId}: ${err?.message || err}`);
      return false;
    }
  }
