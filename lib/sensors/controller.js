...
export function createSensorController({ defs, onRowUpdate, onReading }) {
  ...
  function emitReading(id, reading) {
    ...
    updateReadingThrottled(id, reading);
  }
      ...
      try {
        ...
        if (strategy.id.startsWith("DeviceMotion")) {
          ...
          stopFn = startDeviceMotion({
            onData: (data) => {
              if (strategy.id === "DeviceMotion.acceleration") {
                emitReading(id, data.acceleration);
              } else if (strategy.id === "DeviceMotion.rotationRate") {
                emitReading(id, data.rotationRate);
          ...
          stopFn = startDeviceOrientationQuaternion({
            mode: "relative",
            onData: (data) => emitReading(id, data),
            onError: (err) => updateRow(id, { error: safeError(err) }),
          });
